...
export default function PlanAssociation() {
    ...
    useEffect(() => { }, [planList, planForm, adding]);
    return (
        <>
            <div className="row">
...
function PlanAssociationForm(props) {
    ...
    const checkLengh = (suffix) => {
        if (suffix.target.value.length > 4) {
            setIsError(true);
            // setPinSuffixValue(suffix.target.value);
    ...
    const handleClientGroupChange = (client_group_changed) => {
        setIsClientGroupChanged(true);
        setSelectedClientGroupValue(client_group_changed['client_group_value']);
        setSelectedClientGroupLabel(client_group_changed['client_group_label']);
    }

    useEffect(() => {

        if (props.planForm) {
            if (!isClientGroupChanged) {
                console.log(props.planForm)
                setSelectedClientGroupValue(props.planForm.client_group_id);
                setSelectedClientGroupLabel(props.planForm.client_group_id);
            }
        }
        ...
        transactionType, transactionAssociation, memProcFlagSelected, isError, submitDisable]);
    return (
        <>
