...
function ShowSearchTherapy()
{
    const{register, handleSubmit, watch, formState : {errors}} = useForm();
    const[therapyList, setTherapyList] = useState('');
    const [therapyClass, seTherapyClass] = useState('');
    const showSearchResult = (e) => {
        var arr = [
            {therapy_id : '5621', name : 'therapy one'},
            {therapy_id : '5372', name : 'therapy two'},
            {therapy_id : '8368', name : 'therapy three'}
        ];
        var class_arr = [
        ...
        setTherapyList(arr);
        seTherapyClass(class_arr);
    }
    useEffect(() => {}, [therapyList]);
    useEffect(() => {}, [therapyClass]);
    ...
    return(
                        ...
                        <div className="row mb-2">
                            <div className="col-md-4 mb-3">
                                <div className="form-group">
                                    <small>ID</small>
                                <input type="text" className="form-control" placeholder='Enter Therapy Class ID to search' {...register("therapy_class_id",{required:true})}/>
                                {errors.therapy_class_id && <span><p className='notvalid'>This field is required</p></span>}
                                </div>
                            </div>
        ...
        <ShowTherapyList searchResult={therapyList}  therapyClassList={therapyClass}/>
        </>
    )
}

function ShowTherapyList(props)
{
    const therapyArray = [];
    for(let i=0; i<props.searchResult.length; i++)
    {
        therapyArray.push(<TherapyRow therapyRow={props.searchResult[i]} />);
    }
    const therapyClassArray = [];
    for (let j = 0; j < props.therapyClassList.length; j++) {
        therapyClassArray.push(<TherapyClassRow therapyClassRow={props.therapyClassList[j]} />);
    }
    return(
        <>
        <div className="card mt-3 mb-3">
...
function TherapyRow(props)
{
    return(
        <>
        <tr>
            <td>{props.therapyRow.therapy_id}</td>
            <td>{props.therapyRow.name}</td>
...
function TherapyClassRow(props)
{
    return(
        <>
            <tr>
                <td>{props.therapyClassRow.class}</td>
                <td>{props.therapyClassRow.eff_date}</td>
                <td>{props.therapyClassRow.progress_status}</td>
                <td>{props.therapyClassRow.process_rule}</td>
                <td><button className="btn btn-sm btn-info" id=""><i className="fa fa-eye"></i> View</button></td>
            </tr>
        </>
    )
